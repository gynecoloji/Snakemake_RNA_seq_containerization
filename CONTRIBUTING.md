...
Reference genomes/annotations are not tracked; place them under `ref/` as
described in [`config/README.md`](config/README.md) and the top-level `README.md`.

## Running the workflow

```bash
snakemake -s workflow/Snakefile -n                       # dry run (validates config + DAG)
snakemake -s workflow/Snakefile --use-conda --cores 20   # everything (core → QC → Salmon)
...
`CITATION.cff`. You do **not** edit the changelog or version numbers by hand.
